...
* Features
** Engrammer Layout
Arno's Engram optimized for programmers. Vowels clustered on left hand,
punctuation in center columns.
...
** Home Row Mods
Hold home row keys for modifiers, tap for letters. Bilateral enforcement
prevents same-hand misfires.

| Finger | Left (key) | Right (key) | Modifier |
|--------+------------+-------------+----------|
| Index  | A          | H           | Ctrl     |
| Middle | E          | T           | Shift    |
| Ring   | I          | S           | Alt      |

Cmd is on index fingers below home row (K and R) to reduce pinky strain.
...
** Combos
Ergonomic combos (no lateral finger strain):

| Keys          | Action                    |
|---------------+---------------------------|
| K + R         | Sticky Cmd (one-shot)     |
...
Available recipes:
    ...
    flash  # Copy firmware to mounted bootloader volume

    [dev]
    draw   # Generate keymap visualization
    fmt    # Format project files
    keypos # Print key position indices
